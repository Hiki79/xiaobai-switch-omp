...
export const PiApplyPanel = memo(function PiApplyPanel() {
  ...
  const handleModelChange = (nextModelId: string) => {
    ...
    const levels = piReasoningLevelsForModel(nextModelId);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const result = await apply({
        ...
        piReasoningLevels: reasoningLevels,
        piReasoningLevel: reasoningLevel ?? null,
      });
      showApplyOutcome(
        modal,
        t,
          ...
          {!hasAnySite ? (
            ...
            <Space orientation="vertical" className="w-full" size="middle">
              ...
              <div style={rowStyle}>
                ...
                <div className="mt-1 text-xs opacity-50">{t("apply.piModelHint")}</div>
              </div>
            </Space>
          )}
        </SettingsGroup>
        ...
        {site && (
          <>
            <SettingsGroup title={t("apply.groupPiModels")}>
              <ModelCatalogSection
                title={t("apply.piWriteAllModels")}
                hint={t("apply.piWriteAllModelsHint")}
                models={models}
                loading={modelsLoading}
            ...
            <SettingsGroup title={t("apply.groupPiReasoning")}>
              <ReasoningLevelFields
                levels={reasoningLevels}
                onLevelsChange={setReasoningLevels}
                defaultLevel={reasoningLevel}
                onDefaultLevelChange={setReasoningLevel}
                allowed={piReasoningLevelsForModel(modelId ?? "")}
                defaultLabel={t("apply.piReasoningLevel")}
                defaultHint={t("apply.piReasoningHint")}
                variantsHint={t("apply.piReasoningVariantsHint")}
              />
            </SettingsGroup>
          </>
        )}
